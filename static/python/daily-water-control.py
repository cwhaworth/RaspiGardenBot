...
def readJson(files):
	...
	fList = []
	for file in files:
		with open(f'/var/www/RaspiGardenBot/static/json/{file}.json', 'r') as f:
...
def writeJson(data):
	...
	for d in data['allData']:
		with open(f'/var/www/RaspiGardenBot/static/json/{d["file"]}.json', 'w') as f:
...
def main():
	...
	with open('/var/www/RaspiGardenBot/static/json/forecast.json') as f:
		fcast = json.load(f)
	fcastToday = []
